...
func TestWillTopicConstructor(t *testing.T) {
	...
	assert.Equal("*packets2.WillTopic", reflect.TypeOf(pkt).String(), "Type should be WillTopic")
	assert.Equal(qos, pkt.WillQOS, "Bad WillQOS value")
...
func TestWillTopicUnmarshalInvalid(t *testing.T) {
	...
	if assert.Error(err) {
		assert.Contains(err.Error(), "bad WILLTOPIC2 packet length")
	}
}

func TestWillTopicStringer(t *testing.T) {
	pkt := NewWillTopic("test-topic", 1, true)
	assert.Equal(t, `WILLTOPIC2(WillTopic="test-topic", QOS=1, Retain=true)`, pkt.String())

	// Packet with zero-length topic.
	pkt = NewWillTopic("", 0, false)
	assert.Equal(t, `WILLTOPIC2(WillTopic="")`, pkt.String())
}
