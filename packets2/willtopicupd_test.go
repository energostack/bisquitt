...
func TestWillTopicUpdConstructor(t *testing.T) {
	...
	assert.Equal("*packets2.WillTopicUpd", reflect.TypeOf(pkt).String(), "Type should be WillTopicUpd")
	assert.Equal(qos, pkt.WillQOS, "Bad QOS value")
...
func TestWillTopicUpdUnmarshalInvalid(t *testing.T) {
	...
	buff := bytes.NewBuffer([]byte{
		3,                       // Length
		byte(pkts.WILLTOPICUPD), // PktType
		0,                       // Flags
		// Will Topic missing
	})
	_, err := ReadPacket(buff)
	if assert.Error(err) {
		assert.Contains(err.Error(), "bad WILLTOPICUPD2 packet length")
	}
}

func TestWillTopicUpdStringer(t *testing.T) {
	pkt := NewWillTopicUpd("test-topic", 1, true)
	assert.Equal(t, `WILLTOPICUPD2(WillTopic="test-topic", QOS=1, Retain=true)`, pkt.String())

	// Packet with zero-length topic.
	pkt = NewWillTopicUpd("", 0, false)
	assert.Equal(t, `WILLTOPICUPD2(WillTopic="")`, pkt.String())
}
