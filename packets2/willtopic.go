...
func (p *WillTopic) computeLength() {
	...
	// WillTopic field (i.e. it is exactly 2 bytes long).
	// [MQTT-SN 2.0 specification draft WD20, chapter 2.1.8.3 Will Topic]
...
func (p *WillTopic) Unpack(buf []byte) error {
	...
	case 0:
		p.WillTopic = ""
	case 1:
		return fmt.Errorf("bad WILLTOPIC2 packet length: expected 0 or >=2, got %d",
			len(buf))
	default:
		p.decodeFlags(buf[0])
...
func (p WillTopic) String() string {
	if len(p.WillTopic) == 0 {
		return fmt.Sprintf(`WILLTOPIC2(WillTopic="")`)
	}
	return fmt.Sprintf("WILLTOPIC2(WillTopic=%#v, QOS=%d, Retain=%t)", p.WillTopic, p.WillQOS, p.Retain)
}
