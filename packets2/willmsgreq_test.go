...
func TestWillMsgReqConstructor(t *testing.T) {
	...
	assert.Equal(t, "*packets2.WillMsgReq", reflect.TypeOf(pkt).String(), "Type should be WillMsgReq")
}
...
func TestWillMsgReqUnmarshalInvalid(t *testing.T) {
	...
	buff := bytes.NewBuffer([]byte{
		3,                     // Length
		byte(pkts.WILLMSGREQ), // PktType
		0,                     // junk
	})
	_, err := ReadPacket(buff)
	if assert.Error(err) {
		assert.Contains(err.Error(), "bad WILLMSGREQ2 packet length")
	}
}

func TestWillMsgReqStringer(t *testing.T) {
	pkt := NewWillMsgReq()
	assert.Equal(t, "WILLMSGREQ2", pkt.String())
}
