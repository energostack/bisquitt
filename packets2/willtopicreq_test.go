...
func TestWillTopicReqConstructor(t *testing.T) {
	...
	assert.Equal(t, "*packets2.WillTopicReq", reflect.TypeOf(pkt).String(), "Type should be WillTopicReq")
}
...
func TestWillTopicReqUnmarshalInvalid(t *testing.T) {
	...
	buff := bytes.NewBuffer([]byte{
		3,                       // Length
		byte(pkts.WILLTOPICREQ), // TopicType
		0,                       // junk
	})
	_, err := ReadPacket(buff)
	if assert.Error(err) {
		assert.Contains(err.Error(), "bad WILLTOPICREQ2 packet length")
	}
}

func TestWillTopicReqStringer(t *testing.T) {
	pkt := NewWillTopicReq()
	assert.Equal(t, "WILLTOPICREQ2", pkt.String())
}
