...
func TestWillMsgRespConstructor(t *testing.T) {
	...
	assert.Equal("*packets2.WillMsgResp", reflect.TypeOf(pkt).String(), "Type should be WillMsgResp")
...
func TestWillMsgRespUnmarshalInvalid(t *testing.T) {
	...
	buff := bytes.NewBuffer([]byte{
		2,                      // Length
		byte(pkts.WILLMSGRESP), // PktType
		// Return Code missing
	})
	_, err := ReadPacket(buff)
	if assert.Error(err) {
		assert.Contains(err.Error(), "bad WILLMSGRESP2 packet length")
	}
...
func TestWillMsgRespStringer(t *testing.T) {
	pkt := NewWillMsgResp(RC_CONGESTION)
	assert.Equal(t, "WILLMSGRESP2(ReasonCode=congestion)", pkt.String())
}
